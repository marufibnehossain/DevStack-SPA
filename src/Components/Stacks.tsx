...
const Stacks = ({ promise }: { promise: Promise<IStack[]> }) => {
  ...
  const handleAddToStack = (stack: IStack) => {
    const alreadySelected = selectedStacks.some((item) => item.id === stack.id);
    ...
    <div className="w-full px-[4vw] sm:pb-25 pb-10">
      <div className="max-w-305 w-full mx-auto">
        <div className="sm:mb-10 mb-6">
          <h1 className="lg:text-4xl sm:text-3xl text-2xl sm:text-left text-center font-bold leading-[1.2em] tracking-[-1.5px] font-[Inter] mb-2">
        ...
        <div className="w-full flex sm:flex-row flex-col justify-between gap-8">
